...
class SupplyChainFeedbackEdit extends Component {
  constructor(props) {
    super(...arguments);
    this.state = {
      feedbackId: 0,
    ...
    this.setState({ feedbackId: this.props.match.params.id }, async () => {
      const secondFunction = async () => {
        ...
        $.ajax({
          ...
          error: (error) => {
            alert(
              "Unable to load Supply Chain Feedback. Please try your request again"
            );
            return;
          },
        });
        ...
        $.ajax({
          url: `/api-iec/supplyChainFeedback/${this.state.feedbackId}`, //TODO: update request URL
          type: "GET",
          beforeSend: function (xhr) {
            //Include the bearer token in header
            xhr.setRequestHeader(
          ...
          error: (error) => {
            alert(
              "Unable to load Supply Chain Feedback to edit. Please try your request again"
            );
            return;
          },
        });
  ...
  feedBackDeleteId = (event) => {
    ...
    const secondFunction = async () => {
      ...
      $.ajax({
        ...
        crossDomain: true,
        success: (result) => {
          this.props.history.push("/supplyChainFeedback");
          return;
        },
        error: (error) => {
          $.ajax({
            ...
            error: (error) => {
              alert(
                "Unable to delete the Supply Chain Feedback. Please try your request again"
              );
              return;
            },
          });
  ...
  feedBackEditId = (event) => {
    ...
    if (this.state.feedbackName === "") {
      alert("please add Supply Chain Feedback");
    } else {
      const secondFunction = async () => {
        const result = await this.props.refreshToken();
        // do something else here after firstFunction completes
        ...
        $.ajax({
          ...
          error: (error) => {
            alert(
              "Unable to update Supply Chain Feedback. Please try your request again"
            );
            return;
          },
        });
  ...
  render() {
    return (
      <div>
        <div></div>
        <Header as="h2">
          <Image circular src={feedBack_icon} /> Supply Chain Feedback Edit
        </Header>
...
export default SupplyChainFeedbackEdit;

...
class ProcurementFeedbackEdit extends Component {
  constructor(props) {
    super(...arguments);
    this.state = {
      feedbackId: 0,
    ...
    this.setState({ feedbackId: this.props.match.params.id }, async () => {
      const secondFunction = async () => {
        ...
        $.ajax({
          ...
          error: (error) => {
            alert(
              "Unable to load Procurement Feedback. Please try your request again"
            );
            return;
          },
        });
        ...
        $.ajax({
          url: `/api-iec/procurementFeedback/${this.state.feedbackId}`, //TODO: update request URL
          type: "GET",
          beforeSend: function (xhr) {
            //Include the bearer token in header
            xhr.setRequestHeader(
          ...
          error: (error) => {
            alert(
              "Unable to load Procurement Feedback to edit. Please try your request again"
            );
            return;
          },
        });
  ...
  feedBackDeleteId = (event) => {
    ...
    const secondFunction = async () => {
      ...
      $.ajax({
        ...
        crossDomain: true,
        success: (result) => {
          this.props.history.push("/procurementFeedback");
          return;
        },
        error: (error) => {
          $.ajax({
            ...
            error: (error) => {
              alert(
                "Unable to delete the Procurement Feedback. Please try your request again"
              );
              return;
            },
          });
  ...
  feedBackEditId = (event) => {
    ...
    if (this.state.feedbackName === "") {
      alert("please add Procurement Feedback");
    } else {
      const secondFunction = async () => {
        const result = await this.props.refreshToken();
        // do something else here after firstFunction completes
        ...
        $.ajax({
          ...
          error: (error) => {
            alert(
              "Unable to update Procurement Feedback. Please try your request again"
            );
            return;
          },
        });
  ...
  render() {
    return (
      <div>
        <div></div>
        <Header as="h2">
          <Image circular src={feedBack_icon} /> Procurement Feedback Edit
        </Header>
        ...
        <div id="add_feedBack">
          <Form>
            <Table definition>
              <Table.Body>
                <Table.Row>
                  <Table.Cell width={3}>Procurement Feedback</Table.Cell>
                  <Table.Cell>
                    <input
                      id="input"
...
export default ProcurementFeedbackEdit;

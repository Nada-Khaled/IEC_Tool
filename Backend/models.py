...
#DONE
class Supply_Chain_Feedback(Base):
...
#DONE
class Procurement_Feedback(Base):
...
#DONE
class Decision_Support_Feedback(Base):
...
class IEC(Base):
  ...
  attachment_mimetype = Column(db.Text, nullable = True)
  ...
  def format(self):
    ...
    return {
      ...
      # 'attachment': base64.encodebytes(self.attachment), mimetype = self.attachment_mimetype,
...
class IEC_Dep_Type(Base):
  ...
  total = Column(Float, nullable=True)
                           ...
                           #className,      my_tablename
  PR_id = db.relationship('Procurement_Request',
                          backref='iec_dep_type', lazy=True)
  ...
  # owner_id = Column(Integer, db.ForeignKey('owner.id'), nullable = False)
  product_type_id = Column(Integer, db.ForeignKey('product_type.id'), nullable = False)
  ...
  def __init__(self,item_description,is_foreign_currency,is_capex,is_opex,iec,product_type,
              need_realocation_PR,need_realocation_PO,foreign_currency,owner,capex_egp,opex_egp):
    ...
    self.owner = owner
    self.opex_egp = int(opex_egp)
    self.capex_egp = int(capex_egp)
    self.total_egp = int(capex_egp) + int(opex_egp)
...
class Procurement_Request(Base):
  __tablename__ = 'procurement_request'

  id = Column(Integer, primary_key=True)
  description = Column(String, nullable = False)
  justification = Column(String, nullable = False)
  oracle_id = Column(Integer,  nullable = False)
  comment = Column(String, nullable = False)
  date = Column(DateTime, nullable=False)
                                          # tablename
  status_id = Column(Integer, db.ForeignKey('status.id'), nullable = False)
  iec_dep_type_id = Column(Integer, db.ForeignKey('iec_dep_type.id'), nullable = False)
